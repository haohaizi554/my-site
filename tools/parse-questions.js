...
function isChoiceGroupHeading(line) {
  return /^##\s+一[、.]\s*单选题/.test(line) || /^##\s+一、单选题/.test(line);
}
...
function blockEndFrom(start) {
  ...
  while (end < lines.length) {
    if (isChoiceGroupHeading(lines[end]) || isQuestionHeading(lines[end])) break;
...
function parseChoiceBlock(start, groupIndex) {
  ...
  const section = SECTION_NAMES[groupIndex - 1] || `章节 ${groupIndex}`;
  const prefix = SECTION_PREFIX[groupIndex - 1] || `section${groupIndex}`;
...
function parseJudgementRows() {
  ...
  for (let i = tableStart + 2; i < tableEnd; i += 1) {
    const line = lines[i];
    const match = line.match(/^\|\s*(\d+)\s*\|\s*(.+?)\s*\|\s*([AB])\s*\|\s*(.+?)\s*\|\s*$/);
    if (!match) {
...
for (let i = 0; i < lines.length; i += 1) {
  if (isChoiceGroupHeading(lines[i])) {
    choiceGroup += 1;
    continue;
  }
  if (choiceGroup >= 1 && choiceGroup <= SECTION_NAMES.length && (tableStart < 0 || i < tableStart) && isQuestionHeading(lines[i])) {
    questions.push(parseChoiceBlock(i, choiceGroup));
    i = blockEndFrom(i) - 1;
  }
}
